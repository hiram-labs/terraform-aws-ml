...
|---------------------|----------------------------------------------|-----------------|
| scripts/download_models.py  | Download/upload Hugging Face models to S3     | `python scripts/download_models.py --bucket my-models --model-type whisper --model-names openai/whisper-base,openai/whisper-small` |
| scripts/trigger_jobs.py     | Trigger jobs via SNS (supports presets: extract-audio, transcribe-audio; optional: --input-bucket, --output-bucket, --model-bucket) | `python scripts/trigger_jobs.py --preset transcribe-audio --data '{"input_key": "audio/input.wav"}'` |
